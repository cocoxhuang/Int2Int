...
from dyckwords import area_data, dinv_data, bounce_data, area_dinv_data, bounce_area_data, area_dinv_bounce_data
import time
...
def format_data(task, seq_len, input_file):
    ...
    for line in lines:
        ...
        if task == "area_dinv_bounce":
            ...
            output.append(encode_integer_array(line[seq_len:seq_len+3]))
    # put all in a file
    df = pd.DataFrame()
    df['x'] = [f"V{seq_len} " + x for x in input]
    df['y'] = [f"V2 " + x for x in output]
    # df['y'] = output
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", type=str, help="Prediction task name: area, dinv, bounce, bijection, area_dinv, bounce_area, area_dinv_bounce")
    ...
    gen_data_tasks = ["area", "dinv", "bounce", "area_dinv", "bounce_area", "area_dinv_bounce"] # tasks that the script can generate data for
